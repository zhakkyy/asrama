  ...
  <!--Content Tetap Ditengah-->
    <div class="container">
      ...
      <div class="section no-pad-bot" id="index-banner">
        ...
        <?php
        $qry = mysqli_query($db,"SELECT *FROM soal_tf");
        $no = 1;
        while($row = mysqli_fetch_array($qry))
        {
        ?>
        <td><?php echo $row['idsoal_tf']; ?></td>
        <td><?php echo $row['soal_t'];?></td>
        <td><?php echo $row['soal_f'];?></td>
        <td>
          <a href="admin_editsoaltestf.php?id=<?php echo $row['idsoal_tf']; ?>"><button type="button" class="btn pmd-btn-raised pmd-ripple-effect btn-info">Edit</button></a> <!--untuk soal e-->
        </td>
        </tr>
