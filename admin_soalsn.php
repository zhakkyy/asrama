...
<body>
  ...
  <div class="navbar-fixed">
    ...
    <ul class="side-nav fixed">
      ...
      <li class ="white"><a href="admin_soalei.php"><i class="material-icons">assignments</i>Soal Extrovert & Introvert</a></li>
    ...
    <div class="container">
      ...
      <div class="section no-pad-bot" id="index-banner">
        ...
        <table id="soal" class="table pmd-table table-hover table-striped display responsive nowrap" cellspacing="0" width="100%">
          <tr><th>No.</th>
              <th>Soal Sensing</th>
              <th>Soal Intuitive</th>
              <th></th>

          </tr>

        <?php
        $qry = mysqli_query($db,"SELECT *FROM soal_sn");
        $no = 1;
        while($row = mysqli_fetch_array($qry))
        {
        ?>
        <td><?php echo $row['idsoal_sn']; ?></td>
        <td><?php echo $row['soal_s'];?></td>
        <td><?php echo $row['soal_n'];?></td>
        <td>
          <a href="admin_editsoaltessn.php?id=<?php echo $row['idsoal_sn']; ?>"><button type="button" class="btn pmd-btn-raised pmd-ripple-effect btn-info">Edit</button></a> <!--untuk soal e-->
        </td>
        </tr>
